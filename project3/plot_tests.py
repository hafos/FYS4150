...
if __name__ == "__main__":
    ...
    # fig, ax = plt.subplots(1, 2, figsize=(14/1.4, 7/1.4))
    # for i in range(5):
    ...
    # plt.title('Without Coulomb interaction')
    # plt.legend()
    # plt.axis('equal')
    # plt.tight_layout()
    # plt.savefig('test_results/no_interaction.pdf')
    # plt.show()
    # plt.plot(pos3[0], pos3[1], label='Particle 1')
    # plt.plot(pos4[0], pos4[1], label='Particle 2')
    # plt.xlabel(r'x $[\mu m]$'); plt.ylabel(r'y $[\mu m]$')
    # plt.title('With Coulomb interaction')
    # plt.legend()
    # plt.axis('equal')
    # plt.tight_layout()
    # plt.savefig('test_results/with_interaction.pdf')
    # plt.show()

    """ Problem 9: Test with Two particles with and without Coulomb interactions exploring the phase space """
    fig, ax = plt.subplots(2, 3, figsize=(14/1.2, 9/1.2))
    ax[0, 0].plot(pos1[0], vel1[0], label='Particle 1')
    ax[0, 0].plot(pos2[0], vel2[0], label='Particle 2')
    ax[0, 0].set_ylabel(r'x $[\mu m]$')
    ax[0, 1].plot(pos1[1], vel1[1])
    ax[0, 1].plot(pos2[1], vel2[1])
    ax[0, 1].set_ylabel(r'y $[\mu m]$')
    ax[0, 2].plot(pos1[2], vel1[2])
    ax[0, 2].plot(pos2[2], vel2[2], 'o')
    ax[0, 2].set_ylabel(r'z $[\mu m]$')
    ax[1, 0].plot(pos3[0], vel3[0])
    ax[1, 0].plot(pos4[0], vel4[0])
    ax[1, 0].set_ylabel(r'x $[\mu m]$')
    ax[1, 1].plot(pos3[1], vel3[1])
    ax[1, 1].plot(pos4[1], vel4[1])
    ax[1, 1].set_ylabel(r'y $[\mu m]$')
    ax[1, 2].plot(pos3[2], vel3[2])
    ax[1, 2].plot(pos4[2], vel4[2])
    ax[1, 2].set_ylabel(r'z $[\mu m]$')
    ax[0, 0].set_xlabel(r'$v_x$'' ''$[\mu m /\mu s]$')
    ax[0, 1].set_xlabel(r'$v_y$'' ''$[\mu m /\mu s]$')
    ax[0, 2].set_xlabel(r'$v_z$'' ''$[\mu m /\mu s]$')
    ax[1, 0].set_xlabel(r'$v_x$'' ''$[\mu m /\mu s]$')
    ax[1, 1].set_xlabel(r'$v_y$'' ''$[\mu m /\mu s]$')
    ax[1, 2].set_xlabel(r'$v_z$'' ''$[\mu m /\mu s]$')
    ax[0, 1].set_title('Phase space without Coulomb interaction')
    ax[1, 1].set_title('Phase space with coulomb interaction')
    fig.legend(loc='upper right')
    fig.tight_layout()
    plt.savefig('test_results/phase_space.pdf')
    plt.show()
    # plt.plot(pos1[0], pos1[1], label='Particle 1')
    # plt.plot(pos2[0], pos2[1], label='Particle 2')
    # plt.xlabel(r'x $[\mu m]$'); plt.ylabel(r'y $[\mu m]$')
    ...
    """ Problem 9: Two particle 3D plots """"
